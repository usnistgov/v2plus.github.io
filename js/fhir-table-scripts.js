// modified version of fhir-table-scripts.js
  ...
  for (i = 0; i < rows.length; i++) {
    ...
    if (row.id.startsWith(id)) {
      if (noex && row.id.startsWith(noex)) {
        ...
        if (row.id != id) {
          row.style.display = "";
          if (row.className.endsWith('closed')) {
            noex = row.id;
          }
        }
      }
...
function tableRowAction(src) {
  ...
  let klasses = row.className.split(' ');
  klasses = klasses.filter(e => e !== 'open');
  ...
  // state = "odd open";
  //   }
  if (klasses.includes("closed")) {
    klasses = klasses.filter(e => e !== 'closed');
    klasses.push('open');
    fhirTableRowExpand(table, row.id);
    row.className = klasses.join(' ');
